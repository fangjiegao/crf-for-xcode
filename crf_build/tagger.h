...
class TaggerImpl : public Tagger {
 public:
  explicit TaggerImpl() : mode_(TEST),
                          vlevel_(0), 
                          nbest_(0),  //是否返回Nbest结果
                          ysize_(0),  //标签集合到大小
                          Z_(0),  //
                          feature_id_(0),
                          thread_id_(0),
  ...
  std::string  my_parse_stream(std::istream *is, std::ostream *os);
  std::string  my_parse_stream_str(std::string is, std::ostream *os);
  bool         read(std::istream *is);
  bool         read_arr(std::string str);
  void         close();
 ...
 private:
  ...
  unsigned int    mode_ ;
  unsigned int    vlevel_;
  unsigned int    nbest_;          //是否返回nbest结果
  size_t          ysize_;          //标间集合的大小
  double          cost_;           //目前的cost,目标是让cost最小
  double          Z_;              //p(Y|X)中的h归一化分母
  size_t          feature_id_;     //该Tagger控制的feature_id
  unsigned short  thread_id_;      //所属thread_id
  FeatureIndex   *feature_index_;  //feature_index的引用
  Allocator      *allocator_;      //allocator的引用
  std::vector<std::vector <const char *> > x_;  //训练数据的以每一行的方式存储在x_中
  std::vector<std::vector <Node *> > node_;     //对应图模型中的节点
  std::vector<std::vector<double> > penalty_;   //额外的penalty=0
  std::vector<unsigned short int>  answer_;     //标签序列的标号集合
  std::vector<unsigned short int>  result_;     //模型的预测结果
  whatlog       what_;                          //日志
  string_buffer os_;
