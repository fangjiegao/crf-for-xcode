...
void FeatureIndex::rebuildFeatures(TaggerImpl *tagger) const {
  ...
  FeatureCache *feature_cache = allocator->feature_cache();
    ...
    std::cout << "rebuildFeatures_tagger->size():" << tagger->size() << std::endl;
...
bool FeatureIndex::buildFeatures(TaggerImpl *tagger) const {
  ...
  std::cout << "shrink->FeatureIndex::buildFeatures......" << std::endl;
  ...
  for(int i=0; i<unigram_templs_.size(); i++)
    std::cout << "buildFeatures_>unigram_templs_:" << unigram_templs_[i] << std::endl;
  for(int i=0; i<bigram_templs_.size(); i++)
    std::cout << "buildFeatures_>bigram_templs_:" << bigram_templs_[i] << std::endl;
  
  for (std::vector<std::string>::const_iterator it = unigram_templs_.begin(); it != unigram_templs_.end(); ++it)
    std::cout << "buildFeatures_>unigram_templs_:" << *it << std::endl;
  */
  std::cout << "tagger->size():" <<  tagger->size() << std::endl;
  for (size_t cur = 0; cur < tagger->size(); ++cur) {
      //std::cout << tagger->size() << ":" << cur << std::endl;
    for (std::vector<std::string>::const_iterator it = unigram_templs_.begin(); it != unigram_templs_.end(); ++it) {
        //std::cout << "it->c_str():" << it->c_str() << os << std::endl;
      if (!applyRule(&os, it->c_str(), cur, *tagger)) {
        std::cout << "false" << std::endl;
        return false;
      }
      ADD;  //将上一步抽取的特征转成ID,加入到feature中去
    }
    feature_cache->add(feature);
    /*
    for (int i = 0; i < feature.size();i++)
    {
        std::cout << "feature:" << feature[i] << std::endl;
    }
    */
    feature.clear();
  }
