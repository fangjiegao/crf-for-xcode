...
bool TaggerImpl::add(const char* line) {
  ...
  std::cout << "TaggerImpl::add:" << line << p << std::endl;
...
bool TaggerImpl::read(std::istream *is) {
  ...
  for (;;) {
    // 读取到line的指针中去
      std::cout << "TaggerImpl::read:" << line.size() << line.get() << std::endl;
...
bool TaggerImpl::read_arr(std::string str) {
    // 测试和训练时读取文件
    scoped_fixed_array<char, 8192> line;
    clear();
    ...
    for (size_t i = 0; i<iSize/con_zn_len; i++){
        std::string s = str.substr(i*con_zn_len,con_zn_len);
        std::cout << iSize/con_zn_len << i*con_zn_len << s << ";" << std::endl;
        line.set_template_data(s);
        if (line[0] == '\0' || line[0] == ' ' || line[0] == '\t') {
            break;
        }
        if (!add(line.get())) {
...
void TaggerImpl::buildLattice() {
  ...
  feature_index_->rebuildFeatures(this);  // 将训练序列构建成一个网络图 node和path
...
void TaggerImpl::forwardbackward() {
  ...
  for (int i = 0; i < static_cast<int>(x_.size()); ++i) {
    for (size_t j = 0; j < ysize_; ++j) {
      node_[i][j]->calcAlpha();//后向算法
    }
  }

  for (int i = static_cast<int>(x_.size() - 1); i >= 0;  --i) {
    for (size_t j = 0; j < ysize_; ++j) {
      node_[i][j]->calcBeta();//前向算法
    }
  }
...
double TaggerImpl::gradient(double *expected) {
  ...
  buildLattice();  //rebuildFeatures,将训练序列构建成一个网络图
  forwardbackward();  //前向后向算法
  double s = 0.0;

  for (size_t i = 0;   i < x_.size(); ++i) {
    for (size_t j = 0; j < ysize_; ++j) {
      node_[i][j]->calcExpectation(expected, Z_, ysize_); //计算梯度和目标值
    }
  }

  for (size_t i = 0;   i < x_.size(); ++i) {
    for (const int *f = node_[i][answer_[i]]->fvector; *f != -1; ++f) {
      --expected[*f + answer_[i]];  //减去节点标准答案的特征计数
    }
    s += node_[i][answer_[i]]->cost;  // UNIGRAM cost,累加w * phi(y_i,x)
    const std::vector<Path *> &lpath = node_[i][answer_[i]]->lpath;
    for (const_Path_iterator it = lpath.begin(); it != lpath.end(); ++it) {
      if ((*it)->lnode->y == answer_[(*it)->lnode->x]) {
        for (const int *f = (*it)->fvector; *f != -1; ++f) {
          --expected[*f +(*it)->lnode->y * ysize_ +(*it)->rnode->y];  //减去标准答案的特征计数
        }
        s += (*it)->cost;  // BIGRAM COST,累加w * phi(y_i-1, y_i,x)
        break;
      }
    }
  }
...
bool TaggerImpl::parse_stream(std::istream *is,
  ...
  toString();
  //std::cout << "os_.data():" << os_.data() << std::endl;
  std::cout << "os_.data():" << typeid(os_.data()).name() << std::endl;
  os->write(os_.data(), os_.size());
  return os_.data();
  //return true;
}
    
std::string TaggerImpl::my_parse_stream(std::istream *is, std::ostream *os) {
    if (!read(is) || !parse()) {
        return "";
    }
    if (x_.empty()) {
...
std::string TaggerImpl::my_parse_stream_str(std::string is, std::ostream *os) {
    if (!read_arr(is) || !parse()) {
        return "";
    }
    if (x_.empty()) {
        return "";
...
std::string my_crfpp_test(const Param &param) {
    ...
    std::cout << "rest[i].c_str():" << std::string(rest[0].c_str()) << std::endl;
    CRFPP::istream_wrapper is(rest[0].c_str());
    if (!*is) {
        std::cerr << "no such file or directory: " << rest[0] << std::endl;
        return "";
    }
    std::string res = tagger.my_parse_stream(is.get(), os.get());
    std::cout << "res:" << res << std::endl;
    
    return res;
}
    
std::string my_crfpp_test_str(const Param &param) {
    std::cout <<  "my_crfpp_test_str................" << std::endl;
    if (param.get<bool>("version")) {
        std::cout <<  param.version();
        return "";
    }
    ...
    std::cout << "rest[i].c_str():" << std::string(rest[0].c_str()) << std::endl;
    std::string res = tagger.my_parse_stream_str(std::string(rest[0].c_str()), os.get());
    std::cout << "res:" << res << std::endl;
...
int crfpp_test(const Param &param) {
  ...
  for (size_t i = 0; i < rest.size(); ++i) {
    //遍历每个文件
    std::cout << "rest[i].c_str():" << rest[i].c_str() << std::endl;
    ...
    /*
    while (*is) {
      // std::cout << is.get() << os.get() << std::endl;
...
int crfpp_test(int argc, char **argv) {
    for( int i = 0 ; i < argc; i++)
        std::cout << "crfpp_test:" << argv[i] << std::endl;
  CRFPP::Param param;
  param.open(argc, argv, long_options);
  return CRFPP::crfpp_test(param);
}

int my_crfpp_test(int argc, char **argv) {
    for( int i = 0 ; i < argc; i++)
        std::cout << "my_crfpp_test:" << argv[i] << std::endl;
    CRFPP::Param param;
    param.open(argc, argv, long_options);
    // std::string res = CRFPP::my_crfpp_test(param);
    std::string res = CRFPP::my_crfpp_test_str(param);
    std::cout << "my_crfpp_test->res:" << res << std::endl;
    return 0;
}
