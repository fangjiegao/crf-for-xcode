...
void make_templs(const std::vector<std::string> unigram_templs,
  ...
  std::cout << "make_templs:" << *templs << std::endl;
}
}  // namespace
...
bool EncoderFeatureIndex::openTemplate(const char *filename) {
  ...
  for(int i=0; i<unigram_templs_.size(); i++)
    std::cout << "unigram_templs_:" << unigram_templs_[i] << std::endl;
  for(int i=0; i<bigram_templs_.size(); i++)
    std::cout << "bigram_templs_:" << bigram_templs_[i] << std::endl;
...
bool EncoderFeatureIndex::openTagSet(const char *filename) {
  std::cout << "parse tag -> EncoderFeatureIndex::openTagSet......." << filename << std::endl;
  std::ifstream ifs(WPATH(filename));
  CHECK_FALSE(ifs) << "no such file or directory: " << filename;

  scoped_fixed_array<char, 8192> line;  //动态数组
  scoped_fixed_array<char *, 1024> column;
  size_t max_size = 0;
  std::set<std::string> candset;
  while (ifs.getline(line.get(), line.size())) {  //读到line所定义的地址中,最大的读取长度为line.size()
    std::cout << "line.get():" << line.get() << line.size() << std::endl;
    if (line[0] == '\0' || line[0] == ' ' || line[0] == '\t') {
      continue;
    }
    const size_t size = tokenize2(line.get(), "\t ",
                                  column.get(), column.size());
    // std::cout << column.get() << column.size() << std::endl;
    if (max_size == 0) {
      max_size = size;
    }
    ...
    std::cout << "column:" << column[max_size-1] << std::endl;
  }
  /*
  std::set<std::string>::iterator it;
  for(it=candset.begin ();it!=candset.end ();it++)
  {
